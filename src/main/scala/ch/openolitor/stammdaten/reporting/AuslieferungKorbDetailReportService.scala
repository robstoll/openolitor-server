...
trait AuslieferungKorbDetailReportService extends AsyncConnectionPoolContextAware with ReportService with StammdatenJsonProtocol {
  ...
  def generateAuslieferungKorbDetailReports(fileType: FileType)(config: ReportConfig[AuslieferungId])(implicit personId: PersonId): Future[Either[ServiceFailed, ReportServiceResult[AuslieferungId]]] = {
    generateReports[AuslieferungId, MultiReport[AuslieferungKorbDetailReport]](
      config,
      auslieferungenByIds,
      fileType,
      None,
  ...
  private def name(fileType: FileType)(auslieferung: MultiReport[AuslieferungKorbDetailReport]) = {
    val now = new DateTime()
    s"auslieferung_korbdetail_${now}"
  }

  private def auslieferungenByIds(auslieferungIds: Seq[AuslieferungId]): Future[(Seq[ValidationError[AuslieferungId]], Seq[MultiReport[AuslieferungKorbDetailReport]])] = {
        ...
        stammdatenReadRepository.getMultiAuslieferungReport(auslieferungIds, projektReport) map { auslieferungReport =>
          ...
          val proAbotyp = (auslieferungReport.entries groupBy (groupIdentifier) map {
            ...
            case (abotypName, proDepotTour) =>
              KorbDetailReportProAbotyp(abotypName, proDepotTour.values.sum, (proDepotTour map (p => KorbDetailReportProDepotTour(p._1, p._2))).toSeq)
          }

          val datum = if (!auslieferungReport.entries.isEmpty) auslieferungReport.entries(0).datum else new DateTime()

          val listOfProducts = List[(String, Int, String)]()
          val seqOfProducts = listOfProducts :+ ("caca", 3, "kg")
          val seqOfProducts2 = seqOfProducts :+ ("coco", 1, "kg")
